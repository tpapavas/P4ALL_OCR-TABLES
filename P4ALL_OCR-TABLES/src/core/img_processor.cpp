...
namespace ocrt {
	...
	bool ImageProcessor::ThresholdImage(cv::Mat& output, BinarizationType type, int winx, int winy, double k, double dR) {
		if (img.empty())
			return false;
	 ...
	 * @return
	 */
	double ImageProcessor::CalcLocalStats(cv::Mat& im, cv::Mat& map_m, cv::Mat& map_s, int winx, int winy, double& mean, double& max_s, double& min_s) {
	...
	*/
	void ImageProcessor::ApplyThreshold(cv::Mat im, cv::Mat output, BinarizationType type, int winx, int winy, double k, double dR) {
	 ...
	 */
	void ImageProcessor::PrepareAll(cv::Mat& input, cv::Mat& output, SegmentationBlocks& blocks) {
	...
	void ImageProcessor::PrepareAll(fz_pixmap** fzpxmap, cv::Mat& thres, SegmentationBlocks& blocks) {
		cv::Mat input;
		pixmap2mat(fzpxmap, input);
		PrepareAll(input, thres, blocks);
	}

	void ImageProcessor::PrepareAll(Pix** px, cv::Mat& thres, SegmentationBlocks& blocks) {
		cv::Mat input;
		pix2mat(px, input);
		PrepareAll(input, thres, blocks);
	}
	 ...
	 */
	l_int32  ImageProcessor::DoPageSegmentation(PIX* pixs, SegmentationBlocks& blocks) {
		l_int32      zero;
		BOXA* boxatm, * boxahm;
	 ...
	 */
	void ImageProcessor::ExtractTextImage(cv::Mat& input, SegmentationBlocks& blk, cv::Mat& output) {
	...
	void ImageProcessor::ReorderImage(cv::Mat& input, SegmentationBlocks& blk, cv::Mat& output) {
	...
	bool ImageProcessor::pix2mat(Pix** px, cv::Mat& mat) {
		if (((*px)->d != 8 && (*px)->d != 1)
			|| ((*px)->w < 1)) return false;
	...
	bool ImageProcessor::pixmap2mat(fz_pixmap** fzpxmap, cv::Mat& mat) {
	...
	bool ImageProcessor::ClearImage(cv::Mat& image, cv::Mat& output) {
		...
		ImageProcessor::ThresholdImage(temp_er, temp_th, bin_type, window, window, k);
